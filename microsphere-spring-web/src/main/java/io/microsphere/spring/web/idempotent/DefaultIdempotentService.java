...
public class DefaultIdempotentService implements IdempotentService {
    ...
    @Override
    public boolean invalidateToken(NativeWebRequest request, Idempotent idempotent) {
        String tokenName = idempotent.tokenName();
        REQUEST.removeAttribute(request, tokenName);
        return SESSION.removeAttribute(request, tokenName) != null;
    }

    @Override
    public boolean storeToken(NativeWebRequest request, Idempotent idempotent, String newToken) {
        String tokenName = idempotent.tokenName();
        SESSION.setAttribute(request, tokenName, newToken);
        return true;
    }
}
