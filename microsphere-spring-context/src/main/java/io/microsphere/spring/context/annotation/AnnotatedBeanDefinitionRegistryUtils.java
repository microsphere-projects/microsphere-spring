...
public abstract class AnnotatedBeanDefinitionRegistryUtils implements Utils {
    ...
    public static boolean isPresentBean(BeanDefinitionRegistry registry, Class<?> annotatedClass) {
        ...
        if (logger.isTraceEnabled()) {
            logger.trace("The annotated component[ class : '{}' , bean name : '{}' ] was present in registry : {}",
                    annotatedClass, targetBeanName, registry);
        }
    ...
    public static int scanBasePackages(BeanDefinitionRegistry registry, String... basePackages) {
        ...
        if (isNotEmpty(basePackages)) {
            ...
            for (String scannedBeanName : scannedBeanNames) {
                ...
                if (traceEnabled) {
                    logger.trace("Component [ beab name : '{}' , class : {} ]", scannedBeanName, scannedBeanDefinition.getBeanClassName());
                }
            }
        }
    ...
    public static BeanNameGenerator resolveAnnotatedBeanNameGenerator(BeanDefinitionRegistry registry) {
        ...
        if (beanNameGenerator == null) {
            if (logger.isTraceEnabled()) {
                logger.trace("BeanNameGenerator bean can't be found in BeanFactory with name : {}", CONFIGURATION_BEAN_NAME_GENERATOR);
                logger.trace("BeanNameGenerator will be a instance of {}, it maybe a potential problem on bean name generation.", AnnotationBeanNameGenerator.class.getName());
            }
     ...
     * generates bean names, and returns a set of {@link BeanDefinitionHolder} objects encapsulating the found bean
     * definitions.
     *
