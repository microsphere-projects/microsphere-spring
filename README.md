...
- **Handler method interception** — `HandlerMethodInterceptor` / `HandlerMethodArgumentInterceptor` provide AOP-style hooks around MVC and WebFlux controller invocations
- **Idempotent request support** — `@Idempotent` annotation prevents duplicate HTTP request processing
...
- **Rich testing utilities** — `EmbeddedTomcatContextLoader`, `EnableEmbeddedDatabase`, `AbstractWebFluxTest`, and servlet/MVC test helpers
...
- **Bug reports and feature requests** — search the [existing issues](https://github.com/microsphere-projects/microsphere-spring/issues) first; if not found, [open a new issue](https://github.com/microsphere-projects/microsphere-spring/issues/new) and include your Spring and Java versions, a minimal reproducer, and the full stack trace if applicable
- **Questions and discussions** — use [GitHub Discussions](https://github.com/microsphere-projects/microsphere-spring/discussions)
- **Interactive documentation** — ask questions directly against the codebase via [DeepWiki](https://deepwiki.com/microsphere-projects/microsphere-spring) or [Zread](https://zread.ai/microsphere-projects/microsphere-spring)
